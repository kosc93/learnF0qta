...
PlotFileGenerator::PlotFileGenerator (std::string targetLine, std::string path, std::string subdir, double shift)
{
...
void PlotFileGenerator::generate_plot_file_qta ()
{
	...
	fout << "set output '../../plots/" << m_subdir << "/" << m_name <<".png'" << std::endl;
...
void PlotFileGenerator::generate_plot_file_svm ()
{
	// create directory for plots and output file (plot file)
	std::string path = m_path + "../../plots/";
	const int dir_err = mkdir(path.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
	...
	for (unsigned int i=0; i<m_slope.size(); ++i)
	{
		...
		double f1 = m_offset[i];
		double f2 = m_offset[i] + m_slope[i]*(m_bounds[i+1] - m_bounds[i]);
		fout << "set arrow from " << std::to_string(t1) << "," << std::to_string(f1) << " to " << std::to_string(t2) << "," << std::to_string(f2) << " nohead dt 4 lc '#FF0000'" << std::endl;
	}

	// write f0 curve information
	fout << std::endl << "###### plot curves from data points #####" << std::endl;
	fout << "plot '" << m_name <<".semitonef0' using 2:3 title 'Original F0' with points lt 3 lc '#0000FF' lw 1,\\" << std::endl;
	fout << " '" 	 << m_name <<".qtaf0sampled' using 2:3 title 'qTA F0' with linespoints lt -1 lc black lw 1 dt 2,\\" << std::endl;
	fout << " '" 	 << m_name <<".svmf0sampled' using 2:3 title 'SVM F0' with linespoints lt -1 lc '#FF0000' lw 1" << std::endl;
...
void PlotFileGenerator::analyze_syllable_bounds()
{
	...
	std::string fileName = m_path + "../../corpus/" + m_name + ".TextGrid";
	std::ifstream finF0;
