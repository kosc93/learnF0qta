...
if task = 1
	...
	for current_target from 1 to ntargets
		...
		##### save result
		select PitchTier fittedf0sampledpredicted
		Down to TableOfReal... Hertz
		Write to headerless spreadsheet file... 'directory$''name$'.qtaf0sampledpredicted
		Remove
		select TableOfReal targets-predicted
		Write to headerless spreadsheet file... 'directory$''name$'.targets-predicted
		#Remove
...
procedure qtaAnalysis
	...
	##### save results
	Create PitchTier... fittedf0sampledpredicted wordStart wordEnd
	Create TableOfReal... fittedf0sampledpredicted 1 2
	Set column label (index)... 1 SampleTime
	Set column label (index)... 2 fittedf0 (st)

	Create TableOfReal... targets-predicted 1 6
	...
	for nm from 1 to nintervals
		...
		if not label$ = ""
			...
			for i from 1 to number_samples
				select TableOfReal fittedf0sampledpredicted
				len_buff = length(buff$)
				line_index = index(buff$,newline$)
				sbuff$ = left$(buff$,line_index-1)
				buff$ = mid$(buff$,line_index+1,len_buff)
...
procedure assembleTargetFile
	...
	writeFileLine: "targets-predicted.txt", "slope,offset,strength,duration,rmse,corr"
	...
	for current_file from 1 to numberOfFiles
		...
		if fileReadable ("'name$'.targets-predicted")
			Read TableOfReal from headerless spreadsheet file... 'name$'.targets-predicted
			select TableOfReal 'name$'
			nrows = Get number of rows

			##### iterate over rows (syllable targets)
			appendFile: "targets-predicted.txt",name$
			for i from 1 to nrows
				...
				#write targets
				appendFile: "targets-predicted.txt",",m:",m,",b:",b,",l:",l,",d:",d,",r:",r,",c:",c
			endfor
			appendFile: "targets-predicted.txt",newline$
		endif
	endfor
endproc
...
procedure qtaResynthesis
	...
	nowarn Save as WAV file: "'directory$'./resynthesis/'name$'_svm.wav"
	Remove
	select Manipulation 'name$'
	Remove
endproc
